export const portfolioData = {
  hero: {
    ...
    subtagline: '',
    intro:
      '수학적 사고를 기반으로 RAG, LLM Agent, 모델 최적화 영역을 다뤄왔습니다. 모델을 블랙박스로 두지 않고, 동작 원리에서 출발해 시스템 전체를 설계하는 방식으로 일합니다.',
  ...
  about: {
    title: 'AI를 블랙박스로 다루지 않습니다.',
    // '도메인을 이해하고, 방법론을 선택하고, 설계·구현합니다.',
    description:
      'AI/ML 엔지니어로서 LLM Agent, RAG 시스템, 모델 최적화를 다뤄왔습니다. 수학 전공에서 길러진 분석적 사고와 이론에 대한 깊은 이해가 강점입니다. AI를 블랙박스로 두지 않고 내부 동작 원리에서 출발해 시스템을 설계하며, 모델이 못하는 일은 외부 Tool과 시스템 구조로 보완해 작동하는 결과물을 만듭니다.',
    ...
    highlights: [
      ...
      {
        label: '강점',
        value: '이론적 깊이와 시스템적 사고를 함께 갖춘 문제 해결력',
        // '문제의 핵심을 빠르게 파악하고, 적절한 방식으로 끝까지 구현',
      },
      {
        label: '지향점',
  ...
  projects: [
      ...
      issues: [
        {
          ...
          solution: 'ESG 문서 유형별 Qdrant 컬렉션 분리 설계와 메타데이터 구조를 구성했습니다.',
        },
        {
          problem: '이슈 도출 → 설문 → 중대성 평가 → 리포트 작성까지 전 과정이 수동이었습니다.',
          solution:
            'FastAPI 기반 AI 서비스 아키텍처와 이슈 생성, 보고서 초안, 챗봇에 공통으로 쓰이는 RAG 파이프라인을 구현했습니다.',
        },
        {
          problem: '결과에 어떤 문서가 근거가 되었는지 남지 않으면 실무 신뢰성이 떨어졌습니다.',
          solution: '근거 텍스트와 출처가 함께 남도록 프롬프트 형식을 설계했습니다.',
        },
        {
          problem:
            'Frontend, Spring Boot, AI 서비스가 병렬로 개발되는 상황에서 호출 순서와 데이터 형식이 어긋날 위험이 컸습니다.',
          solution: '데이터 흐름과 API 명세를 정리해 팀 간 인터페이스를 같은 흐름으로 맞췄습니다.',
        },
      ],
      results: [
    ...
    },
    {
      ...
      issues: [
        ...
        {
          problem: '방법론의 수학적 정의를 실제 데이터 파이프라인과 검증 절차로 연결해야 했습니다.',
          solution:
            'Dynamic TDA / fMRI에는 순열검정 기반 통계 검증을 설계하고, 두 방법론의 설명력과 한계를 비교했습니다.',
        },
      ],
      results: [
    ...
    },
    {
      ...
      issues: [
        {
          problem: '원본 모델 대비 크기, 속도, 성능 사이의 균형을 고려해야 했습니다.',
          solution: '경량화 이후 손실된 성능을 복구하기 위한 방법론을 중심으로 실험을 설계했습니다.',
        },
        {
          problem: '특정 추론 환경 안에서 실제로 적용 가능한 방법만 검토해야 했습니다.',
          solution: 'PPL과 downstream task 성능을 분리하고 실제 제출 환경 제약과 함께 해석했습니다.',
        },
        {
          problem: 'Quantization 이후 손실된 성능을 어떤 방식으로 회복할지 비교가 필요했습니다.',
          solution:
            '다양한 데이터 기반 Recovery Fine-tuning을 적용하고 Knowledge Distillation과 비교했습니다.',
        },
      ],
      results: [
    ...
    },
    {
      ...
      issues: [
        {
          problem: '청킹, 임베딩, Top-K를 조정해도 단순 RAG는 복합 재무 질의에서 계속 실패했습니다.',
          solution:
        ...
        {
          problem: '재무 정보의 핵심이 테이블에 있어, 표 형태 그대로는 검색 품질이 충분히 나오지 않았습니다.',
          solution:
            '재무 테이블을 자연어 문장으로 변환하고 회사명, 회계연도 등 메타데이터 필터와 시맨틱 검색을 함께 사용했습니다.',
        },
        {
          problem:
      ...
      results: [
        ...
        '질의 유형별 라우팅과 Tool 분리로 단순 조회와 복합 질의를 서로 다른 경로로 처리하도록 구현했습니다.',
